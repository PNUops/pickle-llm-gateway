...
- **사용량 한도**: Key마다 분당 요청 횟수와 토큰 사용량, 동시 요청 수를 제한합니다.
  네 한도 모두 교내 서빙 모델에만 적용됩니다 — 상용 모델의 통제는 금액 한도입니다.
...
교내 서빙 모델의 축으로, **키 단위 한도 넷이 모두 이 축의 모델에만 적용됩니다** —
분당 요청 횟수와 분당 토큰 사용량, 동시 요청 수, 그리고 문서의 `quotaExhausted`
플래그(일일 토큰 한도)입니다. 넷 다 교내 서빙 용량을 나누는 장치이고, 상용 모델
...
있습니다. 항목은 정확한 모델 이름이거나 벤더 하나를 여는 프리픽스(`openai/*`)이고, 판정은
소문자로 내려 비교하므로 대소문자가 다른 이름으로 목록을 우회할 수 없습니다(위의
`allowedModels`는 대소문자를 구분합니다 — 그쪽은 카탈로그 이름의 정확 일치라서
그렇고, 두 목록이 함께 채워지면 더 엄격한 쪽이 답을 정합니다). `*` 하나만
적는 것은 허용하지 않습니다 — 전체 허용은 목록을 비우는 것으로 표현합니다.
...
**이 목록은 TOKEN 축을 건드리지 않습니다.** 목록이 걸린 Key도 교내 서빙 모델은 그대로
씁니다. 목록은 돈에 대한 울타리이고 교내 서빙에는 두를 돈이 없기 때문입니다. 위의
`allowedModels`와 역할이 다릅니다 — 그쪽은 카탈로그 모델을 큐레이트하는 축이라, 채우면
교내 서빙 모델까지 함께 잠깁니다.

적재할 수 없는 항목이 하나라도 있으면 **그 Key를 통째로 버립니다.** 항목만 빼면 목록이
비어 제한 없음이 되어, 잘못된 울타리가 울타리 없음으로 바뀌기 때문입니다.
...
한도에 걸린 응답에는 `Retry-After`를, 통과한 교내 서빙 모델 응답에는 남은 분당 요청
...
| `LLMGW_DEFAULT_RPM` | `20` | Key에 한도가 없을 때의 분당 요청 수 (교내 서빙 모델 한정) |
| `LLMGW_DEFAULT_TPM` | `20000` | Key에 한도가 없을 때의 분당 토큰 수 (교내 서빙 모델 한정) |
| `LLMGW_DEFAULT_CONCURRENCY` | `2` | Key에 한도가 없을 때의 동시 요청 수 (교내 서빙 모델 한정) |
