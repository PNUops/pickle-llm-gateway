...
- **사용량 한도**: Key마다 분당 요청 횟수와 토큰 사용량, 동시 요청 수를 제한합니다.
  네 한도 모두 자체 서빙 모델에만 적용됩니다 — 상용 모델의 통제는 금액 한도입니다.
...
플래그(일일 토큰 한도)입니다. 넷 다 자체 서빙 용량을 나누는 장치이고, 상용 모델
호출은 그 용량을 쓰지 않습니다. 축은 요청 본문의 모델명으로만 알 수 있으므로
게이트웨이는 본문을 읽은 뒤에 이 한도들을 적용합니다. `CREDIT`은 상용 모델의 축으로, 요청에 **그 Key 전용 업스트림
...
차단 목록 `credit_denied_models`는 허용 목록이 열어 둔 것에서 모델을 다시 빼냅니다. 항목의
...
것에 더해 `reasoning_effort`와 `verbosity`를 받습니다. 상용 제공자가 정의하는 필드이고
...
수를 헤더로 실어 클라이언트가 언제 다시 오면 되는지 알 수 있게 합니다. 상용 모델
