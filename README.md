...
- **경로 허용**: 패스스루 경로는 Key마다 허용 목록으로 엽니다. 목록이 비면 한 경로도 열리지
  않습니다. 자체 서빙 경로인 `/v1/chat/completions`와 `/v1/models`는 이 목록 밖입니다.
...
Key의 `passthroughEndpoints`는 그 Key가 쓸 수 있는 패스스루 경로를 정합니다. 값은 능력
이름이고 현재 어휘는 `images`와 `embeddings` 둘입니다. `images` 하나가
`POST /v1/images`와 `GET /v1/images/models`를 함께 엽니다. 경로가 아니라 능력을 적으므로
벤더가 같은 능력 아래에 하위 경로를 더해도 목록은 그대로 둡니다.
...
않습니다. 그리고 이 필드로는 Key를 버리지 않습니다. 읽을 수 없는 항목은 능력 하나를
빼는 방향이라 안전한 쪽으로 기울고, 이 필드 때문에 Key를 버리면 쓰지도 않던 패스스루
능력 하나로 채팅까지 막히기 때문입니다.
