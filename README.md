...
**자동 선택 모델도 예외입니다.** `openrouter/auto`처럼 **모델을 고르는 이름**은 어떤 모델이
답할지 요청 시점에 정해지지 않고, 외부 공급자는 **고른 모델 기준으로 청구**합니다. 목록으로
판정할 대상이 없으므로 **모델 제한이 걸린 Key에서는 거절합니다.** 제한이 없는 Key는 금액
한도만이 경계였으므로 그대로 쓸 수 있습니다. 거절 문구는 목록을 보라고 하지 않고 **모델
