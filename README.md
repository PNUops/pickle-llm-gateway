...
아래 셋은 패스스루 경로입니다. 요청 본문을 그대로 외부 프로바이더에 전달하고 응답을 그대로
...
**CREDIT 축은 여기에 `reasoning_effort`와 `verbosity` 둘을 더 받습니다.** 외부 공급자가
문서화한 필드이고, 에이전트 도구가 모델 정보를 보고 스스로 붙입니다. 그래서 이 둘을 막으면
추론 가능한 유료 모델이 이 게이트웨이로는 전부 못 쓰게 되고, 호출자가 할 수 있는 일이
없습니다.
...
보낼 수 있는 파라미터는 모델의 예산 축이 정합니다. 유료 모델은 자체 서빙 모델이 받는
것에 더해 `reasoning_effort`와 `verbosity`를 받습니다. 외부 공급자가 정의하는 필드이고
에이전트 도구가 모델 정보를 보고 스스로 붙이기 때문입니다. 자체 서빙 모델에서는 두 필드를
거부하고, 유료 모델에서만 쓸 수 있다는 것을 문구로 알립니다. 어느 축인지는 요청 본문의
모델명이 정하므로, 이 판정은 본문을 읽고 모델을 찾은 뒤에 합니다.
