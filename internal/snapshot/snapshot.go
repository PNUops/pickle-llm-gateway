...
type Options struct {
	...
	// TolerateUnknownTopLevel decides what an unknown top-level member means.
	//
	// For a hand-maintained file it is a typo — `serviceEnable` for
	// `serviceEnabled` would silently leave the service on — so the load fails
	// and the operator finds out. For a document generated by the control
	// plane it is a newer api talking to an older gateway, and refusing the
	// whole document there would freeze authorization: revocations stop
	// arriving because a field this gateway does not need was added. The api
	// must then ship the gateway first for every top-level addition, which is
	// exactly the lockstep the per-entry leniency exists to avoid.
	//
	// So strictness follows the writer: strict for the file, tolerant for the
	// control plane.
	TolerateUnknownTopLevel bool
}
...
func Open(ctx context.Context, source Source, statePath string, log *slog.Logger, opts Options) (*Store, error) {
	s := &Store{
		...
		tolerateTop:   opts.TolerateUnknownTopLevel,
	}
...
func (s *Store) reload(ctx context.Context) error {
	...
	st, err := build(raw, s.known, s.tolerateTop)
	if err != nil {
...
func build(raw []byte, known map[string]bool, tolerateTop bool) (*state, error) {
	...
	// the whole document. Whether an unknown TOP-LEVEL member is an error
	// depends on who wrote the document — see Options.TolerateUnknownTopLevel.
	var env struct {
